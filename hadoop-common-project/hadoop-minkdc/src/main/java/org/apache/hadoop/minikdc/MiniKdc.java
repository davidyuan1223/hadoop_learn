 ...
 * For example,running testcases in parallel that start a KDC each.To accomplish this a single MiniKdc should be
 * used for all testcases running in parallel.
 * MiniKdc default configuration values are:
 *  org.name=EXAMPLE(used to create the REALM)
 *  org.domain-COM(used to create the REALM)]
 *  kdc.bind.address=localhost
 *  kdc.port=0
 *  max.ticket.lifetime=864800000
 *  transport=TCP
 *  debug=false
 */
public class MiniKdc {
    public static final String JAVA_SECURITY_KRB5_CONF="java.security.krb5.conf";
    public static final String SUN_SECURITY_KRB5_CONF="sun.security.krb5.conf";
    ...
    static {
        ...
        DEFAULT_CONFIG.setProperty(MAX_TICKET_LIFETIME,"864800000");
     ...
     * Returns: a MiniKdc default configuration
     */
    ...
    public MiniKdc(Properties conf,File workDir)throws Exception{
        ...
        String realm = orgName.toUpperCase(Locale.ENGLISH) + "." + orgDomain.toUpperCase(Locale.ENGLISH);
    }
    ...
    private void prepareKdcServer() throws Exception {
        ...
        if (conf.getProperty(DEBUG) != null) {
            krb5Debug = getAndSet(SUN_SECURITY_KRB5_CONF, conf.getProperty(DEBUG));
        }
    ...
    public synchronized void stop() {
        if (simpleKdc != null) {
            try {
                ...
                if(conf.getProperty(DEBUG) != null) {
                    System.setProperty(SUN_SECURITY_KRB5_CONF,
