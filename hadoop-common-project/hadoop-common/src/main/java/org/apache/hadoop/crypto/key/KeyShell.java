...
public class KeyShell extends CommandShell {
    ...
    protected int init(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (args[i].equals("create")) {
            ...
            }else if (args[i].equals("invalidateCache")) {
                String keyName = "-help";
                if (moreTokens) {
                    keyName = args[++i];
                }
            ...
            } else if (args[i].equals("-size") && moreTokens) {
                options.setBitLength(Integer.parseInt(args[++i]));
            } else if (args[i].equals("-cipher") && moreTokens) {
                options.setCipher(args[++i]);
            }else if (args[i].equals("description") && moreTokens){
                ...
                if (attr.isEmpty() || val.isEmpty()) {
                    getOut().println("\nAttributes must be in attribute=value from," +
    ...
    private class DeleteCommand extends Command{
        ...
        public boolean validate() {
            ...
            if (interactive){
                try {
                    cont= ToolRunner
                            .confirmPrompt("You are about to DELETE all versions of key "+keyName+" from KeyProvider "+provider+".Continue?");
                    if (!cont) {
    ...
    private class ListCommand extends Command{
        ...
        public void execute() throws Exception {
            try {
                ...
                if (metadata) {
                    final KeyProvider.Metadata[] meat=provider
                            .getKeysMetadata(keys.toArray(new String[keys.size()]));
                    for (int i = 0; i < meat.length; i++) {
                        getOut().println(keys.get(i)+": "+meat[i]);
                    }
                }else {
                    for (String key : keys) {
                        getOut().println(key);
