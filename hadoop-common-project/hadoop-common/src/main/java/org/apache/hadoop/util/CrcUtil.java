...
@InterfaceAudience.LimitedPrivate({"HDFS","Common","MapReduce","Yarn"})
...
public final class CrcUtil {
    ...
    public static void writeInt(byte[] buf,int offset,int value)throws IOException{
        if (offset + 4 > buf.length) {
            throw new IOException(String.format("writeInt out of boundsL buf.length=%d, offset=%d",buf.length,offset));
        }
